...
Codex hedefinde çıktı `Agentbase/.codex/skills/*/SKILL.md` ve `Agentbase/AGENTS.md` olarak üretilir. Codex icin ikinci bootstrap yoktur: `manifest.targets` alanındaki `codex`, Claude canonical çıktısından transform edilecek hedefi ifade eder. İsterseniz transformdan sonra `/codex-verify` çalıştırarak Codex skill frontmatter'ını, `$komut` çağrılarını, path adaptasyonlarını ve hook parity iddiası olmadığını kontrol edebilirsiniz. Sadece Claude Code seçildiyse transform ve Codex verify/adapt adımı atlanır.
